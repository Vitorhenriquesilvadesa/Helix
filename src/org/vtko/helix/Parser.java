...
import static org.vtko.helix.TokenType.*;
...
public class Parser {
    ...
    private Stmt declaration() {
        try {
            ...
            if (match(DEF)) return function("function", false);
    ...
    /**
     * Check kind of current token, if right, return the next token, else, throw a new error.
     *
    ...
    private ParseError error(Token token, String message) {
        Helix.error(token, message);
        return new ParseError();
    }

    /**
     * Iter each TokenType in the argument list to check if correspond on expected types.
     *
